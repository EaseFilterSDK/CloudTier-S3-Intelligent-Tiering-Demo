...
namespace CloudTier.AmazonS3Sup
{
    public class S3Utils
    {
        ...
        public static async Task<DirectoryList> FileListAsync(IAmazonS3 client, AmazonS3SiteInfo siteInfo, DirectoryList dirFileList)
        {
            string remotePath = dirFileList.RemoteDirName;
            ...
            try
            {
                ...
                else
                {
                    ...
                    string bucketName = remotePath.Substring(0, remotePath.IndexOf('/'));
                    remotePath = remotePath.Substring(bucketName.Length + 1);
                    request.BucketName = bucketName;
                    request.Prefix = remotePath;
                    request.Delimiter = "/";
                    ...
                    while (listObjects)
                    {
                        ...
                        foreach (string dir in response.CommonPrefixes)
                        {
                            ...
                            var directoryName = dir.Substring(remotePath.Length);
            ...
            catch (AmazonS3Exception amazonS3Exception)
            {
                ...
                {
                   throw new Exception(string.Format("S3 get file list failed. Error code:{0} message:{1} for directory:{2}", amazonS3Exception.ErrorCode, amazonS3Exception.Message, remotePath));
                }
            }
            catch (Exception ex)
            {
               throw new Exception( "S3 get file list failed with error:" + ex.Message + " for directory:" + remotePath);
            }
